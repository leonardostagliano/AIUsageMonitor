...
public partial class NumberStepper : UserControl
{
    ...
    private void Minus_Click(object sender, RoutedEventArgs e) => Value = Math.Max(Minimum, Value - Step);

    private void Plus_Click(object sender, RoutedEventArgs e) => Value = Math.Min(Maximum, Value + Step);
    ...
    private void Input_PreviewKeyDown(object sender, KeyEventArgs e)
    {
        switch (e.Key)
        {
            ...
            case Key.Up: Value = Math.Min(Maximum, Value + Step); e.Handled = true; break;
            case Key.Down: Value = Math.Max(Minimum, Value - Step); e.Handled = true; break;
        }
    }
}
