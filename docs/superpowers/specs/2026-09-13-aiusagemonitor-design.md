...
Il fallback è però subordinato agli hook. Codex 0.154 emette `subagent_start`/`subagent_stop` una volta approvati i gruppi con `/hooks`, e il suo `agent_id` non coincide necessariamente con l'id del thread figlio (il payload hook di Codex porta `session_id`, `turn_id`, `agent_id`, `agent_type`, `transcript_path`, mentre nei rollout un figlio si identifica con `source.subagent.thread_spawn` — `agent_path` tipo `/root/domain_audit` e `agent_nickname` — e non con l'uuid del thread): con entrambe le sorgenti attive lo stesso figlio verrebbe contato due volte ("al lavoro · 2 agenti" per un solo figlio), il riepilogo dei subagenti avrebbe una voce che nessun rollout può quadrare e l'Idle differito aspetterebbe due stop. Regola: appena una sessione Codex riceve un `SubagentStart`/`SubagentStop` dal bridge, la scansione dei rollout si ferma per quella sessione e i figli già annunciati dal fallback vengono chiusi subito; il fallback torna disponibile solo dopo 6 ore senza eventi hook per quella sessione (`HookEventPump.CodexHookGrace`). L'opzione «Codex: rileva i thread figli dai rollout» (`AppSettings.CodexSubagentFallback`, attiva di default perché senza approvazione dei gruppi i rollout sono l'unica fonte) lo disattiva del tutto, riletta a ogni scansione e quindi senza riavvio dell'app.
...
Ogni riga mostra il **totale** dei token della sessione: per Claude Code `Input + Output + CacheRead + CacheWrite` letti dal transcript (`transcript_path` dei payload hook), contando **una sola `usage` per `requestId`** — lo stesso `requestId` compare su più righe (blocco di testo, `tool_use`…), quindi le righe duplicate dello stesso request non si sommano (sommarle gonfia il totale di circa 2,7 volte); le righe senza `requestId` contano una volta ciascuna. L’usage ripetuta è **identica solo nel transcript principale**: nei transcript dei subagenti le righe intermedie di una risposta in streaming portano una usage parziale (`output_tokens` ancora in crescita, gli altri tre componenti già definitivi) e l’ultima riga porta quella finale — su 586 transcript di subagente misurati, 571 hanno almeno un request in cui prima e ultima riga differiscono. Il contatore tiene quindi la usage **più grande** per `requestId` (somma solo la crescita di una riga successiva, mai un delta negativo: su 18 736 request nessuna è risultata non monotona). Non basta contare la riga con `stop_reason` non nullo: 4 317 di quei request non ne hanno nessuna. Per Codex il totale è il `total_tokens` dell'ultimo `event_msg/token_count` del rollout (`info.total_token_usage`, cumulativo per thread).
...
- Tipi di installazione: sviluppo (non file singolo), cartella non scrivibile, piattaforma non Windows x64 → la nuova versione viene segnalata ma installazione integrata disattivata; altrimenti supportata. La prova di scrittura nella cartella dell'exe (file sonda) si fa solo quando un controllo trova una versione da proporre e prima di download e installazione, mai al caricamento: niente avvisi di "Accesso controllato alle cartelle" a ogni avvio.
- Installazione: nuova verifica dell'hash e della versione scritta nell'eseguibile; `ExecutableSwap` copia la nuova versione accanto all'exe (`.new-<guid>`), ne verifica lo SHA-256, rinomina l'exe in esecuzione in `.old-<guid>` e mette la copia al suo posto; avvio della nuova versione con `--updated <pid>`; su errore ripristino dell'exe precedente. L'app si chiude subito (`Shutdown` → `OnExit`). Se l'app viene chiusa durante lo scambio (Esci, fine sessione), `UpdateService.Dispose` annulla l'operazione (l'installer si ferma se lo scambio non e' ancora iniziato) oppure aspetta fino a 30 s che lo scambio finisca o venga ripristinato; a scambio concluso non riavvia la nuova versione.
- Download e installazione girano su un thread del pool (`Task.Run`) e anche l'avvio dell'updater (`UpdatePrompt.Start`, `StartAsync`): lettura della sessione, DPAPI e prova di scrittura non passano mai dal thread UI.
